	...
	if(!empty($array)){ 
	  ...
	  { 
		...
		if($item['parent_id']==0){
			...
			if(is_array($item['children']) && count($item['children']) > 0){ $icon=""; }else{ $icon=false;}
			...
			if(is_array($item['children']) && count($item['children']) > 0){ $attr='class="dropdown-toggle" data-toggle="dropdown"'; }else{ $attr=false;}
			echo '<li '.$class.'> 
					<a  href="'.$url.'" '.$attr.'>
						'.ucwords(@$item['label']).' 
						'.$icon.'
	...
	if(!empty($array)){ 
	  foreach ($array as $item)
	  {
		...
		if($item['type']=='other')
			...
			if(is_array($item['children']) && count($item['children']) > 0){ $icon=""; }else{ $icon=false;}
			if(is_array($item['children']) && count($item['children']) > 0){ $class='class="dropdown""'; }else{ $class=false;}
			if(is_array($item['children']) && count($item['children']) > 0){ $attr='class="dropdown-toggle" data-toggle="dropdown"'; }else{ $attr=false;}
			echo '<li '.$class.'> 
					<a  href="'.$url.'" '.$attr.'>
						'.ucwords(@$item['label']).' 
...
if(!empty($array)){
	  ...
	  foreach ($array as $item)
	  {
		...
		if($item['parent_id']==0){
			if(is_array($item['children']) && count($item['children']) > 0){ $icon="<i class='fa fa-angle-down'></i>"; }else{ $icon=false;}
			if(is_array($item['children']) && count($item['children']) > 0){ $class='class="dropdown"'; }else{ $class=false;}
			echo '
