...
function check_availability($check_in, $check_out, $adults, $kids, $room_type_id)
{
	...
	$CI->db->where('room_status', 'Available');
	...
	foreach ($period as $dt) {
		...
		if ($total_rooms > 0) {
			...
			if (count($orders) >= $total_rooms) {
				...
				$CI->session->set_flashdata('error', "This date is unavailable please select another date.");
	...
	foreach ($period as $dt) {
		...
		if ($total_rooms > 0) {
			if (count($orders) > $total_rooms) {
				$CI->session->unset_userdata('booking_data');
				$CI->session->unset_userdata('coupon_data');
				return 'This date is unavailable please select another date.';
			} else {
				continue;	// continue loop
			}
		} else {
