...
class Account extends Front_Controller {
	...
	function cancel($id){
		...
		if(!empty($_POST['cancel'])){	//cancel booking
			...
			$refundexpiry = date('Y-m-d H:i:s', strtotime($data['bookings']->ordered_on.' + 7 days'));
			$today = date("Y-m-d H:i:s");
			if($refundexpiry >= $today){
				$Token = json_decode($this->paypal_lib->paypal_auth());
				//echo $Token->access_token.'<hr>';
				$refund = $this->paypal_lib->paypal_refund($Token->access_token,$data['bookings']->txn_id);
				if(!empty($refund)){
					$savepayment['remarks'] = 'Refunded';
					$this->book_model->update_payment_by_order($savepayment,$id);
				}
				$this->session->set_flashdata('message', 'Your Booking Is Cancelled with Refund');

			}else{

				$this->session->set_flashdata('message', 'Your Booking Is Cancelled');
