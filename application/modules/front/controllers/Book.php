...
class Book extends Front_Controller {
	...
	{		
		...
		$this->load->model(array('homepage_model','book_model'));
		 ...
		 $this->output->nocache();
		 $this->front_user = $this->session->userdata('front_user');
		 //echo json_encode($this->front_user);
	}
	...
	function step1(){
		...
		//echo '<pre>'; print_r($_POST);die;
			...
			$this->form_validation->set_rules('date_from', 'lang:check_in', 'required');
        ...
        {	
			...
			if(!empty($pay))
			{
				...
				if ($this->form_validation->run() == true)
				{
					...
					//echo 'session <pre>'; print_r($this->session->userdata('booking_data'));die;	
	...
	function pay(){
				...
				if($booking_data['payment_gateway']==3){	// Pay On Arrival
					...
					$this->session->set_flashdata('message', "Your Booking Payment is Success");
					redirect('front/book/order');
				}
				//Set variables for paypal form
				$returnURL = site_url().'front/book/success'.$data['booking_data']['order_id']; //payment success url
	...
	function success($orderid = ''){
		...
		$this->book_model->update_order($save,$id);
		$data['order']	=	$this->book_model->get_order($orderid);
	...
	function mail_booking($order_id){
		...
		//echo $this->mailer->print_debugger(array('headers'));
		return true;
	}	
