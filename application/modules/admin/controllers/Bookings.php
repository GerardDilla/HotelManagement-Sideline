...
class Bookings extends Admin_Controller {
	...
	{		
		parent::__construct();
		$this->load->model(array('booking_model','room_type_model','guest_model','account_model'));
	...
	function refund(){
		...
		$order	=	$this->account_model->get_order($orderid);
		...
		$refund = $this->paypal_lib->paypal_refund($Token->access_token,$order->txn_id);
		if(!empty($refund)){
			$save['remarks'] = 'Refunded';
			$this->booking_model->update_payment($save,$paymentid);
		}
		$this->session->set_flashdata('message', 'Your Booking Is Cancelled with Refund');
		redirect('admin/Bookings/booking/'.$orderid);

		//echo $orderid.' : '.$paymentid;
